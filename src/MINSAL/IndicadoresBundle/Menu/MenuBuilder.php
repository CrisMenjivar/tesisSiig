    ...
    public function createSidebarMenu()
    {
        ...
        foreach ($this->pool->getAdminGroups() as $name => $group) {
            foreach ($group['items'] as $item) {
			    ...
			    if (in_array($ruta, array('menu_reportes'))) 
			    {
                            	if ($admin->hasRoute('menu_reportes') and ( $usuario->hasRole('ROLE_SUPER_ADMIN') or $usuario->hasRole('ROLE_USER_REPORTES'))) {
                                    $menu[$name]
                                            //->addChild('reportes', array('uri' => $admin->generateUrl('menu_reportes')))
                                            ->setExtra('translationdomain', $admin->getTranslationDomain())
                                            ->setExtra('admin', $admin)
                                    ;
                                }
