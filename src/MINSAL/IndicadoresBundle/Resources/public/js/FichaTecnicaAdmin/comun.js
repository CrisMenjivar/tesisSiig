...
// *************************************************************************************************************************************
	...
	$('.area_grafico').each(function() {
			var zona_actual = "grafico_" + zona_contador;
			var serializer = new XMLSerializer();
			var xmlString = serializer.serializeToString(d3.select('#' + zona_actual + ' svg').node());
			var D3imgData = 'data:image/svg+xml;base64,' + btoa(xmlString);
			//******var canvas = document.createElement("canvas");
			//var canvas = document.getElementById("canvas-reportes");
			...
			var indicador = $('#' + zona_actual + ' .titulo_indicador').html();
			var dimension = $('#' + zona_actual + ' .dimension').html();
    			var titulo = indicador + ' (Por '+dimension+')'; 
			//******wrapText(ctx, titulo_indicador, 0, 20, canvas_width, canvas_line);
				//ctx.fillText(titulo_indicador, 0,10);
			//******var dataURL = canvas.toDataURL("image/png");
		
			//var graph = dataURL.replace("data:", "");
			//alert(graph);
			//var dataJSON=JSON.stringify(dataURL);
			
			var datasetPrincipal = JSON.parse($('#' + zona_actual).attr('datasetPrincipal'));
    			var tablaR = construir_tabla_reportes(zona, datasetPrincipal);
			
			//alert(tablaR); 

			form.append(jQuery('<input>', {
				'name': 'data'+zona_contador,
				'value': D3imgData,
				'type': 'hidden'
			}));
			form.append(jQuery('<input>', {
				'name': 'titulo'+zona_contador,
				'value': titulo,
				'type': 'hidden'
			}));
			form.append(jQuery('<input>', {
				'name': 'tablaDatosR'+zona_contador,
				'value': tablaR,
				'type': 'hidden'
			}));
			zona_contador++;
		...
		//$(location).attr("href", url_redir); 
		/*$.ajax({  
		type: "POST",  
		url: url_redir, //path to controller action  
		data: dataURL
		});*/
	    ...
	    function construir_tabla_reportes(zona, datos) {
	    var tabla_datos = '<table border="1" style="margin: 0 auto;">';
	    $.each(datos, function(i, fila) {
		if (i === 0) {
		    // Los nombres de los campos
		    tabla_datos += '<thead><tr>';
		    for (var campo in fila) {
		        ...
		        if (campo === 'category')
		            campo = campo = $('#' + zona + ' .dimension').html();
		        else if (campo === 'measure')
